  ...
  <div>
    ...
    <div v-else>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div v-if="recentProjects.length === 0" class="p-6">
          <UiEmptyState
              title="Belum ada proyek"
              description="Silakan buat proyek pertama Anda untuk mulai mengelola tugas."
          />
        </div>
        ...
        <div v-else class="divide-y divide-gray-50">
          <div v-for="project in recentProjects" :key="project.id" class="px-6 py-4 hover:bg-gray-50 transition-colors flex justify-between items-center">
            ...
            <NuxtLink :to="`/projects/${project.id}`" class="px-3 py-1.5 text-sm bg-white border border-gray-200 rounded-lg hover:bg-gray-50 font-medium text-gray-700 transition-colors">
              Buka Papan
            </NuxtLink>
          </div>
        </div>
